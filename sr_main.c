...
int main(int argc, char **argv)
{
    ...
    uint32_t tcp_est_timeout = 7440;
    uint32_t tcp_trans_timeout = 300;
    ...
    while ((c = getopt(argc, argv, "hs:v:p:u:t:r:l:T:nI:E:R:")) != EOF)
    {
        switch (c)
        {
            ...
            case 'T':
                ...
                break;
            case 'n':
            	nat_is_on = 1; // Turn on the NAT functionality
            	break;
            case 'I': // ICMP query timeout interval [seconds]
            	icmp_query_timeout = atoi((char *)optarg);
            	break;
            case 'E': // TCP established idle timeout [seconds]
            	tcp_est_timeout = atoi((char *)optarg);
                break;
            case 'R': // TCP transitory idle timeout [seconds]
    ...
    sr_init(&sr,nat_is_on,icmp_query_timeout,tcp_est_timeout,tcp_trans_timeout);
